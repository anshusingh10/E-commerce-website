...
export default function Navbar() {
    ...
    // ⭐ ADDED: CALL API WHEN COMPONENT LOADS
    useEffect(() => {
        dispatch(getSettings())
    }, [])


    // ⭐ FIXED: ALWAYS SET DATA EVEN IF ARRAY EMPTY
    useEffect(() => {
        setData({
                ...
                <div className="row gx-0 align-items-center">
                    <div className="col-lg-10 col-6 text-center text-lg-start mb-2 mb-lg-0">
                        <div className="d-flex flex-wrap">
                            ...
                            <Link to={data.googleMap} target='_blank' rel='noreferrer'
                                className="text-muted small me-4">
                                <i className="fas fa-map-marker-alt text-primary me-2"></i>
                                <span className='d-none d-xl-inline-block'>{data.address}</span>
                            </Link>

                            <Link to={`tel:${data.phone}`} target='_blank' rel='noreferrer'
                                className="text-muted small me-4">
                                <i className="fas fa-phone-alt text-primary me-2"></i>
                                <span className='d-none d-xl-inline-block'>{data.phone}</span>
                            </Link>

                            <Link to={`https://wa.me/${data.whatsapp}`} target='_blank' rel='noreferrer'
                                className="text-muted small me-4">
                                <i className="fab fa-whatsapp text-primary me-2"></i>
                                <span className='d-none d-xl-inline-block'>{data.whatsapp}</span>
                            </Link>

                            <Link to={`mailto:${data.email}`} target='_blank' rel='noreferrer'
                                className="text-muted small me-0">
                                <i className="fas fa-envelope text-primary me-2"></i>
                                <span className='d-none d-xl-inline-block'>{data.email}</span>
                            </Link>
                                ...
                                localStorage.getItem("login") ?
                                    ...
                                    <div className="dropdown">
                                        ...
                                        <div className="dropdown-menu rounded">
                                            ...
                                            <Link
                                                to={localStorage.getItem("role") === "Buyer" ? "/profile" : "/admin"}
                                                className="dropdown-item">
                                                <i className="fas fa-user-alt me-2"></i> My Profile
                                            </Link>

                                            <Link to="/cart" className="dropdown-item">
                                                <i className="fas fa-comment-alt me-2"></i> Cart
                                            </Link>
